...
class Dataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.split_ix = {'train': [], 'validation': [], 'test': []}
        for ix in range(len(self.info['images'])):
            img = self.info['images'][ix]
            if not 'split' in img:
                self.split_ix['train'].append(ix)
                self.split_ix['validation'].append(ix)
                self.split_ix['test'].append(ix)
            elif img['split'] == 'train':
                self.split_ix['train'].append(ix)
            elif img['split'] == 'validation':
                self.split_ix['validation'].append(ix)
            elif img['split'] == 'test':
                self.split_ix['test'].append(ix)
        ...
        print('assigned %d images to split val' %len(self.split_ix['validation']))
