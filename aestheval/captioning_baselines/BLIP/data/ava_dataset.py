...
def prepare_json(image_root, split):
    ...
    for i, c in enumerate(json.load(open(image_root+"/processed_%s.json" % split, 'r'))):
        captions = [remove_URL(k) for k in c['comments']]
        imgs.append({'image_id': str(i), 'image': c['im_path'], 'caption': captions})
    return imgs


class ava_train(Dataset):
...
class ava_caption_eval(Dataset):
