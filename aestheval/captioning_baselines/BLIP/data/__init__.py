...
from data.pccd_dataset import pccd_train, pccd_caption_eval
from data.reddit_dataset import reddit_train, reddit_caption_eval
from data.ava_dataset import ava_train, ava_caption_eval
...
def create_dataset(dataset, config, min_scale=0.8):
    ...
    elif dataset=='caption_AVA':
        train_dataset = ava_train(transform_train, config['image_root'], config['ann_root'], prompt=config['prompt'])
        val_dataset = ava_caption_eval(transform_test, config['image_root'], config['ann_root'], 'validation')
        test_dataset = ava_caption_eval(transform_test, config['image_root'], config['ann_root'], 'test')   
        return train_dataset, val_dataset, test_dataset
    
    elif dataset=='caption_PCCD':
        train_dataset = pccd_train(transform_train, config['image_root'], config['ann_root'], prompt=config['prompt'])
        val_dataset = pccd_caption_eval(transform_test, config['image_root'], config['ann_root'], 'validation')
        test_dataset = pccd_caption_eval(transform_test, config['image_root'], config['ann_root'], 'test')   
        return train_dataset, val_dataset, test_dataset

    elif dataset=='caption_reddit':
        train_dataset = reddit_train(transform_train, config['image_root'], config['ann_root'], prompt=config['prompt'])
        val_dataset = reddit_caption_eval(transform_test, config['image_root'], config['ann_root'], 'validation')
        test_dataset = reddit_caption_eval(transform_test, config['image_root'], config['ann_root'], 'test')   
        return train_dataset, val_dataset, test_dataset
